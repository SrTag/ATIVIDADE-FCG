...
//#endif
	...
	float dx, dy;
	GLuint VAODino = setupSprite(5, 4, dx, dy);

	//cout << dx << "  " << dy << endl;
	GLuint texID3 = generateTexture("../textures/sprito.png");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::translate(model, glm::vec3(tilemap.GetPosBloco().x , tilemap.GetPosBloco().y, 0));
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
	...
	if (key == GLFW_KEY_SPACE && action == GLFW_PRESS) //norte
	{
		
		cout << "VALOR DO GETMAP = " << tilemap.getMap(posPersonagem.y, posPersonagem.x) << endl;
		//iAnimation++;
		switch (tilemap.getMap(posPersonagem.y,posPersonagem.x))
		{
		case 5:

...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//DEFINIÇÃO DA MATRIZ MODELO (transf. na geometria)
		glm::mat4 model = glm::mat4(1); // matriz identidade

		//(1) transladando a posição do triangulo
		model = glm::translate(model, glm::vec3(50.0f, 500.0f, 0.0f));
		//(2)rotação
		...
		// PONTOS - GL_POINTS
		//glUniform4f(colorLoc, 0.0f, 0.0f, 1.0f, 1.0f); //(cor dos PONTOS OU LINHAS)enviando cor para variável uniform inputColor
		...
		//(1) transladando a posição do triangulo
		model = glm::translate(model, glm::vec3(50.0f, 500.0f - 50, 0.0f));
		...
		shader.setVec4("inputColor", 1.0f, 0.1f, 0.0f, 1.0f);
		glBindVertexArray(VAO);
		glDrawArrays(GL_TRIANGLES, 0, 6);
		...
		for (int Linha = 1; Linha <= 10; Linha++) 
		{
			for (int Coluna = 1; Coluna <= 10; Coluna++) 
			{
				model = glm::mat4(1);
				//(1) transladando a posição do triangulo
				model = glm::translate(model, glm::vec3(50.0f * Coluna, 500.0f - (50*Linha), 0.0f));
				//(2) scale
				model = glm::scale(model, glm::vec3(50.0f, 50.0f, 1.0f));
				...
				// (218,165,32)- AMARELO
				// (0,191,255)-AGUA
				// (210,105,30)-LARANJA BICO
				// (0,0,0) - OLHO PRETO
				// (255,250,250) - BRANCO BK
				//-------------------------------------------------------

				if (Linha == 1) 
				{
					CorR = CorB = CorG = 250.0f;
				}
				...
				shader.setVec4("inputColor", CorR, CorG, CorB, 1.0f);
				glBindVertexArray(VAO);
				glDrawArrays(GL_TRIANGLES, 0, 6);
